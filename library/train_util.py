...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        def normalize_prompt_text(raw_text: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
            ...
            system_prompt = None
            if prompt_split_token in normalized:
                before, after = normalized.split(prompt_split_token, 1)
        ...
        def parse_repeat_strategy_file(file_path: str) -> Dict[str, int]:
            ...
            try:
                with open(file_path, "rt", encoding="utf-8") as f:
                    for raw_line in f:
                        line = raw_line.strip()
                        if not line or line.startswith("#") or line.startswith("repeat_strategy"):
                            continue
                        if line.startswith("-"):
                            ...
                            repeat_part = parts[-1]
                            if (artist_part.startswith("'") and artist_part.endswith("'")) or (
                                artist_part.startswith('"') and artist_part.endswith('"')
                            ):
                                artist_part = artist_part[1:-1]
                            try:
                                repeat_value = int(float(repeat_part))
                            except ValueError:
                                logger.warning(f"Invalid repeat value '{repeat_part}' in {file_path}, skipping.")
                                continue
                            if repeat_value < 1:
                                logger.warning(f"Repeat value for artist '{artist_part}' in {file_path} is less than 1, skipping.")
                                continue
                            strategy[artist_part] = repeat_value
            except FileNotFoundError:
                return {}
            except Exception as e:
                logger.warning(f"Failed to read repeat strategy file {file_path}: {e}")
                return {}
            return strategy

        def load_repeat_strategy(subset: DreamBoothSubset) -> Dict[str, int]:
            if not self.is_training_dataset:
                return {}
            attributes = subset.custom_attributes or {}
            mapping: Dict[str, int] = {}
            direct_strategy = attributes.get("repeat_strategy")
            if isinstance(direct_strategy, dict):
                for artist_key, repeat_value in direct_strategy.items():
                    try:
                        repeat_int = int(repeat_value)
                    except (TypeError, ValueError):
                        logger.warning(f"Invalid repeat value '{repeat_value}' for artist '{artist_key}' in custom attributes.")
                        continue
                    if repeat_int < 1:
                        logger.warning(f"Repeat value for artist '{artist_key}' in custom attributes is less than 1, skipping.")
                        continue
                    mapping[str(artist_key)] = repeat_int

            repeat_file_attr = attributes.get("repeat_strategy_file")
            candidate_files = []
            if isinstance(repeat_file_attr, str) and repeat_file_attr:
                candidate_files.append(repeat_file_attr)
            candidate_files.append(os.path.join(subset.image_dir, "repeat_strategy.yaml"))
            ...
            for candidate in candidate_files:
                ...
                if not os.path.isabs(candidate_path):
                    candidate_path = os.path.join(subset.image_dir, candidate_path)
                if os.path.isfile(candidate_path):
                    file_mapping = parse_repeat_strategy_file(candidate_path)
                    if file_mapping:
                        logger.info(
                            f"Loaded repeat strategy for subset '{subset.image_dir}' from {candidate_path}: {len(file_mapping)} entries."
                        )
            ...
            for cap_path in cap_paths:
                if os.path.isfile(cap_path):
                    with open(cap_path, "rt", encoding="utf-8") as f:
                        ...
                        caption, system_prompt = normalize_prompt_text(raw_caption)
                    break
            return caption, system_prompt


        def load_dreambooth_dir(subset: DreamBoothSubset, default_repeat: int):
            ...
            if use_cached_info_for_subset:
                ...
                for meta in metas.values():
                    ...
                    if resolution is None:
                        ...
                        sizes.append(tuple(resolution))
                    caption_value = meta.get("caption")
                    caption_text, system_prompt = (
                        normalize_prompt_text(caption_value) if caption_value is not None else ("", None)
                    )
                    if system_prompt:
                        system_prompts.add(system_prompt)
                    captions.append(caption_text if caption_text is not None else "")
                missing_captions: List[str] = []
            else:
                base_dir_path = pathlib.Path(subset.image_dir)
                if subset.recursive:
                    img_paths = [str(p) for p in glob_images_pathlib(base_dir_path, recursive=True)]
                else:
                    img_paths = glob_images(subset.image_dir, "*")
                    if not img_paths:
                        try:
                            has_subdir = any(child.is_dir() for child in base_dir_path.iterdir())
                        except FileNotFoundError:
                            has_subdir = False
                        if has_subdir:
                            img_paths = [str(p) for p in glob_images_pathlib(base_dir_path, recursive=True)]
            ...
            if system_prompts:
                existing_prompts = subset.custom_attributes.get("system_prompts")
                if existing_prompts is None:
                    subset.custom_attributes["system_prompts"] = sorted(system_prompts)
                else:
                    merged = set(existing_prompts if isinstance(existing_prompts, (list, tuple, set)) else [existing_prompts])
                    merged.update(system_prompts)
                    subset.custom_attributes["system_prompts"] = sorted(merged)
            else:
                subset.custom_attributes.setdefault("system_prompts", [])
            ...
            if not use_cached_info_for_subset and subset.cache_info:
                ...
                logger.info(f"cache image info done for / 画像情報を書き出しました: {info_cache_file}")

            repeat_strategy_map = load_repeat_strategy(subset) if self.is_training_dataset else {}
            subset_root = pathlib.Path(subset.image_dir)
            repeat_counts: List[int] = []

            def resolve_artist_key(image_path_str: str) -> str:
                image_path = pathlib.Path(image_path_str)
                try:
                    relative = image_path.relative_to(subset_root)
                    if relative.parts:
            ...
            for img_path in img_paths:
                if self.is_training_dataset:
                    artist_key = resolve_artist_key(img_path)
                    repeat_value = repeat_strategy_map.get(artist_key, default_repeat)
                    try:
                        repeat_int = int(repeat_value)
                    except (TypeError, ValueError):
                        logger.warning(
                            f"Invalid repeat value '{repeat_value}' for image '{img_path}', fallback to {default_repeat}."
                        )
                        repeat_int = default_repeat
                    if repeat_int < 1:
            ...
            return img_paths, captions, sizes, repeat_counts
        ...
        for subset in subsets:
            default_repeat = subset.num_repeats if self.is_training_dataset else 1
            if self.is_training_dataset and default_repeat < 1:
                logger.warning(
                    f"default num_repeats for subset with image_dir='{subset.image_dir}' is less than 1 / num_repeatsが1を下回っているため1に置き換えます: {default_repeat}"
                )
                default_repeat = 1
            ...
            img_paths, captions, sizes, repeat_counts = load_dreambooth_dir(subset, default_repeat)
            if len(img_paths) < 1:
                logger.warning(
            ...
            repeat_total = sum(repeat_counts) if repeat_counts else 0
            if subset.is_reg:
                num_reg_images += repeat_total
            else:
                num_train_images += repeat_total

            for img_path, caption, size, repeat in zip(img_paths, captions, sizes, repeat_counts):
                info = ImageInfo(img_path, repeat, caption, subset.is_reg, img_path)
